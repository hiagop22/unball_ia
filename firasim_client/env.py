...
class MarkovDecisionProcess:
    ...
    def reset_random_init_pos(self, fixed_ball_pos=False):
        self.init_time = time.time()

        self.set_entities_positions(fixed_ball_pos)

        self.send_positions2fira()

        return self.process_state()

    def set_entities_positions(self, fixed_ball_pos):
        if fixed_ball_pos:
            ball_pos = [0,0]
        else:
            ball_pos = [x(), y()]
        ...
        for id in range(self.num_allies_in_field):
            if id != 0:
                ...
                self.ally_robots[id].pos = [-0.3,0, 0]
        
        for id in range(self.num_opponents_in_field):
            if id != 0:
                self.opponent_robots[id].pos = [*init_pos["opponents"][id], theta()]
            else:
                self.opponent_robots[id].pos = [0.3, 0, 0]
    ...
    def done(self):
        ...
        return 1 if abs(self.ball.pos[0]) > abs(self.field.ally_goal_pos[0]) else 0
